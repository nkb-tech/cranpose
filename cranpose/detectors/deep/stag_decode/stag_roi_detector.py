...
class StagDetector:
    ...
    def detect_rois_batch(self, images_in, scale = 1):
        ...
        #  pred features       
        features_preds = self.predictor.predict(images_in, scale)
        self.cnn_timing = self.predictor.cnn_timing
        self.predition_timing = time.time() - t0

        tag_results = []
        for features_pred in features_preds:
            # sparse unordered corners, bbox with 5 anchors (center + 4 ordered corners)
            image_res = detect_center_and_corners(features_pred, min_center_score = min_center_score, min_corner_score = min_corner_score)
            

            # center_to_corner_links and tags (4 ordered corners)
            tag_res, center_to_corner_links = center_and_corners_to_tags(
                image_res, is_allow_no_corner_refine=is_allow_no_corner_refine, is_with_corner_refine = is_with_corner_refine)
