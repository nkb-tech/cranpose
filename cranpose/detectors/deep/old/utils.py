...
def get_box_specs(image_size, stride_list, aspect_ratios= [2, 3], min_max_factors = [4, 8]):
    # specs = [
    ...
    #     ]
    # s = max(image_size[0], image_size[1])
    specs= []
    for stride in stride_list:
        ss1 = image_size[0]//stride
        ss2 = image_size[1]//stride
        spec = SSDSpec((ss1, ss2), stride, SSDBoxSizes(int(stride*min_max_factors[0]), int(stride*min_max_factors[1])), aspect_ratios)
        specs.append(spec)
    return specs
...
def generate_ssd_priors(specs: List[SSDSpec], image_size, clamp=True) -> torch.Tensor:
    ...
    priors = []
    for spec in specs:
        if type(image_size)== list or type(image_size)== tuple:
            scale = [image_size[1] / spec.shrinkage, image_size[0] / spec.shrinkage]
        else:
            scale = [image_size / spec.shrinkage, image_size / spec.shrinkage]
        ...
        for j in range(spec.feature_map_size[0]):
            for i in range(spec.feature_map_size[1]):
                ...
                # h = w = size / min(image_size[0], image_size[1])
                h = size / image_size[0]
                w = size / image_size[1]
                priors.append([
                    x_center,
...
def assign_priors(gt_boxes, gt_labels, corner_form_priors,
    ...
    ious = iou_of(gt_boxes.unsqueeze(0)[..., :4], corner_form_priors.unsqueeze(1)[..., :4])
    # size: num_priors
