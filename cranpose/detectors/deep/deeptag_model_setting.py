...
def load_deeptag_detector_model(tag_family, checkpoint_dir, device = None):
    if device is None:
    ...
    return model_detector, device, tag_type, grid_size_cand_list


def load_deeptag_models(tag_family, checkpoint_dir, device = None):
    if device is None:
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    model_decoder = decoder(num_channels=num_channels_refiner, num_masks=num_masks, num_classes = num_classes)
    state_dict_2stg = torch.load(os.path.join(checkpoint_dir, model_filename_2stg), map_location=device)
    model_decoder.load_state_dict(state_dict_2stg)
    model_decoder.to(device)
    #print(model_decoder)  
